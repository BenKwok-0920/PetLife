...
#pragma mark - LORefreshHeaderGIF 类
...
-  (void)adjustStateWithContentOffset
{
    if (_footerState == LORefreshFooterStateRefreshing) {
        return;
    }
    //当前的偏移量
//    CGFloat contentOffsetY = self.scrollView.contentOffset.y;
    //contentsize的 height
    CGFloat contentSizeHeight = self.scrollView.contentSize.height > self.scrollView.frame.size.height ? self.scrollView.frame.size.height : self.scrollView.contentSize.height;
    
    if ((contentSizeHeight - self.scrollView.contentSize.height + self.scrollView.contentOffset.y) <= 0) {
        self.hidden = YES;
    } else{
        self.hidden = NO;
    }
...
//    CGFloat maxOffsetY = contentOffsetY + self.scrollView.bounds.size.height;
    //如果没滑动到最底部,什么都不做
    if ((contentSizeHeight - self.scrollView.contentSize.height + self.scrollView.contentOffset.y) <= 40){
        if(_footerState == LORefreshFooterStatePulling && !self.scrollView.dragging){
            self.footerState = LORefreshFooterStateRefreshing;
            //            NSLog(@"加载更多");
        } else {
            self.footerState = LORefreshFooterStateIdle;
    ...
    if (self.scrollView.dragging) {
        if((contentSizeHeight - self.scrollView.contentSize.height + self.scrollView.contentOffset.y) > 40 && _footerState == LORefreshFooterStateIdle){
